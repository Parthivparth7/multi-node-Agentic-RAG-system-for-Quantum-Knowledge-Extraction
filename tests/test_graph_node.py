...
def test_extract_concepts_and_relations():
    rec = {
        ...
        "chunk": "DNA RNA Qubit Entanglement BellState Grover Quantum Fourier Transform are linked.",
    }
    concepts = extract_concepts(rec["chunk"])
    assert "Qubit" in concepts
    rels = build_relations_from_record(rec)
    assert rels
    assert {"from", "to", "source", "weight", "domain", "rel_type"}.issubset(rels[0].keys())
